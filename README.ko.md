...
파일 편집에는 별도의 블록을 사용합니다. Codex의 `apply_patch`는 `PATH`에 있는 프로그램이 아니라 Codex CLI에
내장되어 있고 `[EXEC_REQUEST]`의 명령은 한 줄뿐이므로, 모델은 대신 패치가 담긴 `[APPLY_PATCH]` 블록을
출력합니다. 터미널에는 패치 전체가 담긴 **AI PATCH PROPOSAL**이 표시되고, Enter/`y`를 누르면 `PATH`에 있는
`codex` CLI로 적용됩니다(`n`/Esc는 거부, 런처의 승인 팝업에서는 패치를 편집할 수 있습니다). 패치가 건드리는
모든 파일은 워크스페이스 루트 안에 있어야 하며, 밖의 경로는 확인 없이 차단됩니다. `--hitl-auto-approve`에서는
명령과 마찬가지로 패치도 즉시 적용됩니다. 패치는 약 100 KB(Windows에서는 30 KB)로 제한되므로 더 큰 변경은
모델이 나누어 보냅니다.
