...
コマンドは**ワークスペースルート**に限定されます。これは `serve` を実行したディレクトリ、または
`--workspace` で指定したディレクトリです（Codex はデーモンに現在開いているプロジェクトを伝えない
ため、同じプロジェクトを指定してください）：

```bash
codex-chatgpt-web serve --hitl --workspace D:\path\to\your\project
```

リスクを許容する場合、`--hitl-auto-approve` は 1 コマンドごとの確認を完全にスキップします。
...
されます。モデルには理由が伝えられるため、相対パスで再試行できます。
...
ファイルの編集には別のブロックを使います。Codex の `apply_patch` は `PATH` 上のプログラムではなく Codex CLI
に組み込まれており、`[EXEC_REQUEST]` のコマンドは 1 行だけなので、モデルは代わりにパッチを含む
`[APPLY_PATCH]` ブロックを出力します。ターミナルにはパッチ全体を含む **AI PATCH PROPOSAL** が表示され、
Enter/`y` を押すと `PATH` 上の `codex` CLI 経由で適用されます（`n`/Esc で拒否。ランチャーの承認ポップアップでは
パッチを編集できます）。パッチが触れるすべてのファイルはワークスペースルート内に収まる必要があり、
外側のパスは確認なしでブロックされます。`--hitl-auto-approve` ではコマンドと同様にパッチも即座に適用されます。
パッチは約 100 KB（Windows では 30 KB）までなので、大きな変更はモデルが分割します。
