...
test("a Bigger Context HITL commit message also carries the [EXEC_REQUEST] protocol", () => {
  const compiled = compileChatGptWebPrompt(
    request("high"),
    { localToolsEnabled: false, solAvailable: true, proAvailable: true, extraHighAvailable: false },
    undefined,
...
test("HITL cannot be combined with Luna rolling checkpoint capture in one compiled prompt", () => {
  ...
  expect(() => compileChatGptWebPrompt(
    luna,
    { localToolsEnabled: false, solAvailable: false, proAvailable: false, extraHighAvailable: false },
    undefined,
